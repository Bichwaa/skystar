<template>
    <Modal @close-modal="close">
        ...
        <form class="lg:px-6 w-full">
            <p class="font-medium flex items-center mb-3">Update Tax Invoice</p>
            ...
            <div class="grid place-items-center">
                <button  type="submit" @click.prevent="submitForm" 
                    ...
                    <span v-if="!formLoading">Update Invoice</span>
...
function editParticular(doc){
    currentParticular.value = doc
    revealParticularsFields.value = true
}
...
async function getConsignments(){
  ...
  async function updateParticular(){
    formLoading.value = true
    const res = await $axios.patch(`/api/invoice-particulars/${currentParticular.value.ID}`,{...currentParticular.value})
...
async function submitForm(){
    ...
    const newPayload = {
    consignmentId: payload.value.consignmentId,
    referenceNumber:payload.value.referenceNumber,
    invoiceNumber:payload.value.invoiceNumber,
    currency:payload.value.currency,
    vat:payload.value.vat,
}
    const res = await $axios.patch(`/api/tax-invoice/${props.docdata.ID}`,newPayload)
    formLoading.value = false
    console.log(res)
    if(res.status==200 || 201){
        emit("close", res.data)
...
onMounted(async()=>{
    if (props.docdata.ID!=0){
        payload.value = {...props.docdata}
        particulars.value = props.docdata.Particulars
    }
