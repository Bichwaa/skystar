...
<Modal @close-modal="close" class="w-full">
    <h2 class="text-xl text-center font-bold mb-6">Approve Cost</h2>
    ...
    <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
        <div class="flex flex-col items-start justify-center pt-5 pb-6">
            ...
            <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-medium">Requested by:</span> <br/> {{ cost.Requested.firstName }}  {{ cost.Requested.lastName }}</p>
            <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-medium">Amount:</span> <br/> {{ cost.amount }} ({{ cost.currency }})</p>
            <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-medium">Purpose:</span> <br/> {{ cost.purpose }} </p>
...
const consignment = computed(() =>{
     return props.consignments?.filter(consignment => consignment.ID == props.cost.consignmentId)[0]
    })
...
async function upload(){
    ...
    try{
        const res = await $axios.patch(`/api/expenses/update/${props.cost.ID}`,{approvedId:store.user.ID})
        loading.value = false;
