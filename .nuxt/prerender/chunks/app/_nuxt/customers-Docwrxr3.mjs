...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(_ctx.roles, (role, i) => {
      ...
      ssrRenderList(staff.value, (st, i) => {
        _push(`<li class="my-6 flex items-center space-x-2"><a href="#" class="relative block"><img alt="Maurice Lokumba"${ssrRenderAttr("src", _imports_4)} class="mx-auto h-10 w-10 rounded-full object-cover"></a><div class="flex flex-col"><span class="ml-2 text-sm font-semibold text-gray-900 my-2">${ssrInterpolate(st.fullName)}</span><span class="ml-2 text-sm text-gray-400 my-2">${ssrInterpolate(st.email)}</span><span class="ml-2 text-sm text-gray-700 my-2"> Account Overseer: <span class="capitalize text-medium">${ssrInterpolate(st.accountManager)}</span></span></div></li>`);
      });
...
//# sourceMappingURL=customers-Docwrxr3.mjs.map
