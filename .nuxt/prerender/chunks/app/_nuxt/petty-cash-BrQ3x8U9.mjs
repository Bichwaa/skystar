...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex flex-wrap sm:flex-row gap-8" }, _attrs))}><div class="w-full"><div class="mb-4"><div class="w-full rounded-2xl bg-white p-4 shadow-lg"><div class="mb-6 flex items-center justify-between"><div class="flex items-center justify-between w-full"><div class="flex flex-col"><span class="ml-2 font-bold text-black">Petty Cashbook</span><span class="ml-2 text-xs text-gray-500"> For the month of </span></div><button class="rounded-full border border-gray-200 p-1">`);
      _push(ssrRenderComponent(_component_IconsAddIcon, {
        class: "h-7 w-7 cursor-pointer",
        onClick: ($event) => showCreateCustomerForm.value = true
      }, null, _parent));
      _push(`</button></div></div><div class="mb-4 flex items-center justify-between space-x-12"><span class="flex items-center rounded-md bg-green-50 px-2 py-1 text-xl font-semibold text-green-700">${ssrInterpolate(unref(formattedDate))}</span></div><div class="summary flex flex-col gap-3 my-4"><div class="flex items-center justify-start gap-5"><span class="font-medium text-sm">Spent:</span><span class="font-semibold text-sm">Tsh ${ssrInterpolate("420,000")}</span></div><div class="flex items-center justify-start gap-5"><span class="font-medium text-sm">Float:</span><span class="font-semibold text-sm">Tsh ${ssrInterpolate("1,500,000 ")}</span></div><div class="flex items-center justify-start gap-5"><span class="font-medium text-sm">Overspent:</span><span class="font-semibold text-sm">Tsh ${ssrInterpolate(0)}</span></div></div><div class="m-auto block"><table class="w-full divide-y divide-gray-200"><thead class="bg-gray-100"><tr class=""><th class="px-4 py-2 text-left">Requested By</th><th class="px-4 py-2 text-left">Amount</th><th class="px-4 py-2 text-left">Purpose</th><th class="px-4 py-2 text-left">Date requested</th><th class="px-4 py-2 text-left">Actions</th></tr></thead><tbody><!--[-->`);
      ssrRenderList(items, (item) => {
        _push(`<tr><td class="px-4 py-2">${ssrInterpolate(item.requestedBy)}</td><td class="px-4 py-2">${ssrInterpolate(item.amount)}</td><td class="px-4 py-2">${ssrInterpolate(item.purpose)}</td><td class="px-4 py-2">${ssrInterpolate(item.cashoutDate)}</td><td class="flex items-center gap-6 px-4 py-2"><span class="text-[#292a5e] text-sm font-medium hover:text-black duration-300 cursor-pointer">View</span><span class="text-[#d4af37] text-sm font-medium hover:text-black duration-300 cursor-pointer"> Edit </span><span class="text-red-600 text-sm font-medium hover:text-black duration-300 cursor-pointer">Delete</span></td></tr>`);
      });
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/petty-cash.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=petty-cash-BrQ3x8U9.mjs.map
